    ...
    if is_first(t)
        for annual_year = 2015:(gettime(t))
            ...
            for r in d.region
                ...
                v.yagg_periodspan_ann[yr] = 1 #p.yagg_periodspan[t]
        ...
        for annual_year = (gettime(t-1)+1):(gettime(t))
            ...
            for r in d.region
                ...
                v.yagg_periodspan_ann[yr] = 1 #p.yagg_periodspan[t]*fraction_timestep + p.yagg_periodspan[t-1]*(1-fraction_timestep)
            end
        end
    end
end
...
@defcomp EquityWeighting begin
    ...
    pop_population = Parameter(index=[time, region], unit="million person")
        # from Population, needs interpolation
    pop_population_ann = Variable(index=[year, region], unit="million person") # NEW - for annual version
    ...
    act_percap_adaptationcosts = Parameter(index=[time, region], unit="\$/person")
        # from TotalAbatementCosts, need interpolation
    tct_percap_totalcosts_total_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    act_adaptationcosts_total_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    act_percap_adaptationcosts_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    ...
    cons_percap_aftercosts = Parameter(index=[time, region], unit="\$/person")
        # from GDP (first), SLRDamages (last), need interpolation
    cons_percap_consumption_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    cons_percap_aftercosts_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    ...
    wact_partiallyweighted = Variable(index=[time, region], unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    wtct_percap_weightedcosts_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    eact_percap_weightedadaptationcosts_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    wact_percap_partiallyweighted_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    wact_partiallyweighted_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    ...
    pct_g_partiallyweighted_global = Variable(index=[time], unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    pct_percap_partiallyweighted_ann = Variable(index=[year, region], unit="\$/person") # NEW - for annual version
    pct_partiallyweighted_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    pct_g_partiallyweighted_global_ann = Variable(index=[year], unit="\$million") # NEW - for annual version
    ...
    popgrw_populationgrowth = Parameter(index=[time, region], unit="%/year")
        # from RCPSSPScenario, need interpolation
    grw_gdpgrowthrate_ann = Variable(index=[year, region], unit="%/year") # NEW - for annual version
    popgrw_populationgrowth_ann = Variable(index=[year, region], unit="%/year") # NEW - for annual version
    ...
    dfc_consumptiondiscountrate = Variable(index=[time, region], unit="1/year")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    dr_discountrate_ann = Variable(index=[year, region], unit="%/year") # NEW - for annual version
    yp_yearsperiod_ann = Variable(index=[year], unit="year") # NEW - for annual version
    dfc_consumptiondiscountrate_ann = Variable(index=[year, region], unit="1/year") # NEW - for annual version

    df_utilitydiscountrate = Variable(index=[time], unit="fraction")
        # origin in this component, no need for in-component interpolation, as it is calculated in run_timestep
    df_utilitydiscountrate_ann = Variable(index=[year], unit="fraction") # NEW - for annual version
    ...
    pcdt_g_partiallyweighted_discountedglobal = Variable(index=[time], unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    pcdt_partiallyweighted_discounted_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    pcdt_g_partiallyweighted_discountedglobal_ann = Variable(index=[year], unit="\$million") # NEW - for annual version
    ...
    tpc_totalaggregatedcosts_ann = Variable(unit="\$million") # NEW - for annual version
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    pcdat_partiallyweighted_discountedaggregated_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version

    wacdt_partiallyweighted_discounted = Variable(index=[time, region], unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    wacdt_partiallyweighted_discounted_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version

    # Equity weighted impact totals
    rcons_percap_dis = Parameter(index=[time, region], unit="\$/person")
        # from Discontinuity
    rcons_percap_dis_ann = Parameter(index=[year, region], unit="\$/person") # NEW - for annual version
    ...
    widt_equityweightedimpact_discounted = Variable(index=[time, region], unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    wit_equityweightedimpact_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    widt_equityweightedimpact_discounted_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version

    yagg_periodspan = Parameter(index=[time], unit="year")
        # from GDP, needs interpolation
    yagg_periodspan_ann = Variable(index=[year], unit="year") # NEW - for annual version
    ...
    addt_gt_equityweightedimpact_discountedglobal = Variable(unit="\$million")
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    addt_equityweightedimpact_discountedaggregated_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version
    ...
    td_totaldiscountedimpacts_ts = Variable(index=[time], unit="\$million") # for analysis / debugging / code analysis
    td_totaldiscountedimpacts_ann = Variable(unit="\$million") # NEW - for annual version
    td_totaldiscountedimpacts_ann_yr = Variable(index=[year], unit="\$million") # for analysis / debugging / code analysis
    ...
    tac_totaladaptationcosts_ann = Variable(unit="\$million") # NEW - for annual version
        # origin in this component, no need for in-component interpolation, as it is calculated in calc_equityweighting
    aact_equityweightedadaptation_discountedaggregated_ann = Variable(index=[year, region], unit="\$million") # NEW - for annual version


    # Final result: total effect of climate change
    te_totaleffect = Variable(unit="\$million")
    te_totaleffect_ann= Variable(unit="\$million")
    te_totaleffect_ann_yr = Variable(index=[year], unit="\$million") # for analysis / debugging / code analysis
    ...
    function run_timestep(p, v, d, tt)
        ...
        v.td_totaldiscountedimpacts_ts[tt] = v.td_totaldiscountedimpacts # NEW - for comparing intermediary variables
